 ...
 */
public class OldSensorClass implements Sensor {
  private final FileSystem fileSystem;
  private final PathResolver pathResolver;
  private final AnalysisWarnings analysisWarnings;


  public OldSensorClass(FileSystem fileSystem, PathResolver pathResolver, @Nullable AnalysisWarnings analysisWarnings) {
  ...
  private static final Logger LOGGER = Loggers.get(OldSensorClass.class);

  public static List<ModuleInfo> readModuleInfoListFromFile(String filePath) throws IOException {
    Gson gson = new Gson();
    JsonParser parser = new JsonParser();

    try (FileReader reader = new FileReader(filePath)) {
        JsonElement jsonElement = parser.parse(reader);
        JsonArray modulesArray = jsonElement.getAsJsonObject().getAsJsonArray("modules");

        List<ModuleInfo> moduleInfoList = new ArrayList<>();
        for (JsonElement moduleElement : modulesArray) {
            ModuleInfo moduleInfo = gson.fromJson(moduleElement, ModuleInfo.class);
...
public static Optional<Module> findModuleDataBySource(List<Module> moduleList, String source) {
  ...
  public void execute(SensorContext context) {
    ...
    try {
      List<ModuleInfo> moduleInfoList = readModuleInfoListFromFile("./dependencies.json");
      ...
      for (InputFile file : files) {
        ...
        Integer cAvalue = 0;
        Integer cEvalue = 0;
        Optional<ModuleInfo> module =  findModuleBySource(moduleInfoList,file.relativePath());
        if(module.isPresent()){
